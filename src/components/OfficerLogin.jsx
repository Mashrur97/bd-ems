...
export default function OfficerLogin({ role, title, subtitle, icon, accentColor, redirectTo, demo }) {
  ...
  const handleLogin = () => {
    const officer = OFFICERS.find((o) => o.id === id && o.pin === pin && o.role === role);
    if (officer) {
  ...
  return (
    <div className="min-h-screen bg-[#06090f] text-white flex items-center justify-center relative">
      ...
      <div style={{ position: "relative", zIndex: 1 }} className="w-96 p-10 bg-black/50 backdrop-blur rounded-3xl border" style={{ borderColor: accentColor + "33" }}>
        <button
          onClick={() => navigate("/")}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="text-[10px] text-white/20 text-center py-1">
            Demo: <b className="text-white/40">{demo.id}</b> · <b className="text-white/40">{demo.pin}</b>
          </div>
