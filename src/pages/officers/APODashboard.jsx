...
export default function APODashboard() {
  ...
  const { booths, candidates, fetchMyBooths, fetchPublicResults, submitBooth } = useElection();
  ...
  useEffect(() => {
    if (!officer) { navigate("/officer/apo"); return; }
    fetchMyBooths();
    fetchPublicResults();
  }, []);
  ...
  useEffect(() => {
    if (candidates.length > 0) {
      const init = {};
      candidates.forEach((c) => { init[c.candidateId] = ""; });
      setCandVotes(init);
    }
  }, [candidates]);
  ...
  const validate = (issuedVal, usedVal, cvotes) => {
    ...
    const candTotal = Object.values(cvotes).reduce((a, b) => a + (parseInt(b) || 0), 0);
  ...
  const handleValidate = () => {
    ...
    if (submitted.length > 0 && parseInt(issued) > 0) {
      const avg = submitted.reduce((a, b) => a + b.used / b.issued, 0) / submitted.length;
  ...
  const handleSubmit = async () => {
    ...
    const cv = {};
    candidates.forEach((c) => { cv[c.candidateId] = parseInt(candVotes[c.candidateId]) || 0; });
    setSubmitting(true);
    try {
  ...
  return (
    <div className="min-h-screen bg-[#06090f] text-white flex flex-col">
      <Navbar title="ASST. PRESIDING OFFICER" subtitle={`Station ID: ${officer.stationId}`} backTo="/" />
      <div className="flex flex-1">
        ...
        <div className="flex-1 p-4 md:p-8 overflow-auto pb-24 md:pb-8">
          ...
          {section === "entry" && (
            <div>
              <div className="flex items-center gap-2 text-2xl font-bold mb-1">
                <PenSquare size={24} className="text-yellow-400" /> Booth Vote Entry
              </div>
              ...
              <div className="bg-white/[0.03] border border-yellow-500/20 rounded-2xl p-5 mb-5">
                <div className="text-[10px] tracking-widest text-yellow-400 mb-3">SELECT BOOTH</div>
              ...
              {selectedBooth && (
                <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6">
                  <div className="text-[10px] tracking-widest text-white/30 mb-1">ENTERING FOR</div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-5">
                    <div>
                      ...
                      <input
                        type="number"
                        value={issued}
                        onChange={(e) => { setIssued(e.target.value); handleValidate(); }}
                        placeholder="e.g. 800"
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        value={used}
                        onChange={(e) => { setUsed(e.target.value); handleValidate(); }}
                        placeholder="e.g. 642"
                        className="w-full px-4 py-3 rounded-xl bg-white/[0.07] border border-white/10 text-white text-sm outline-none focus:border-yellow-500/50 transition-all placeholder:text-white/20"
                      />
                    </div>
                  ...
                  <div className="text-[10px] tracking-widest text-white/30 mb-3">CANDIDATE VOTES</div>
                  <div className="flex flex-col gap-3 mb-5">
                    {candidates.map((c) => (
                      <div key={c.candidateId} className="flex items-center gap-4">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const updated = { ...candVotes, [c.candidateId]: e.target.value };
                  ...
                  {fraudWarn && (
                    <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl px-4 py-3 mb-4 text-orange-400 text-xs">
                      ⚠ Anomaly detected: turnout deviation exceeds 20% from station average. Please double-check.
                    </div>
                  )}

                  {errors.length === 0 && used && Object.values(candVotes).some((v) => v) && (
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className={`flex-[2] py-3 rounded-xl font-bold text-sm transition-all ${
                        !submitting && errors.length === 0 && used && Object.values(candVotes).some((v) => v)
          ...
          {section === "booths" && (
            <div>
              <div className="flex items-center gap-2 text-2xl font-bold mb-6">
                <Building2 size={24} className="text-yellow-400" /> My Assigned Booths
              </div>
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <div className="text-xs text-white/30 mt-1">Issued: {b.issued} ballots</div>
                      </div>
                      <span
          ...
          {section === "submitted" && (
            <div>
              <div className="flex items-center gap-2 text-2xl font-bold mb-6">
                <CheckCircle size={24} className="text-yellow-400" /> Submitted Results
              </div>
              {submittedBooths.length === 0 ? (
                <div className="text-center py-16 text-white/30">No booths submitted yet.</div>
              ) : (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {candidates.map((c) => {
                        ...
                        const cv = b.candidateVotes instanceof Map
                          ? b.candidateVotes.get(String(c.candidateId))
                          : b.candidateVotes?.[c.candidateId] ?? b.candidateVotes?.[String(c.candidateId)] ?? 0;
                        return (
