...
export default function PODashboard() {
  ...
  const { booths, stations, incidents, verifyStation, reportIncident, totalVotes, turnout } = useElection();
  const [section, setSection] = useState("overview");
  const [showModal, setShowModal] = useState(false);
  const [officer] = useState(() => JSON.parse(sessionStorage.getItem("currentOfficer")));

  if (!officer) { navigate("/officer/po"); return null; }
  ...
  const stationTotal = submittedBooths.reduce((a, b) =>
    a + Object.values(b.candidateVotes).reduce((x, y) => x + y, 0), 0
  );
  ...
  const sidebarItems = [
    ...
    { id: "verify", icon: "🔍", label: "Verify Booths" },
    { id: "voters", icon: "👤", label: "Voter List" },
    ...
    <div className="min-h-screen bg-[#06090f] text-white flex flex-col">
      ...
      <div className="flex flex-1">
        ...
        <div className="flex-1 p-4 md:p-8 overflow-auto pb-24 md:pb-8">
          ...
          {section === "overview" && (
            <div>
              <div className="text-2xl font-bold mb-6">📋 Station Overview</div>
              <div className="grid grid-cols-3 gap-4 mb-6">
                {[
                  { icon: "🗳️", val: stationTotal.toLocaleString(), label: "STATION VOTES", color: "text-yellow-400", border: "border-yellow-500/20" },
                  { icon: "📈", val: ((stationTotal / 2400) * 100).toFixed(1) + "%", label: "TURNOUT", color: "text-green-400", border: "border-green-500/20" },
                  { icon: "🏫", val: `${submittedBooths.length}/${stationBooths.length}`, label: "BOOTHS DONE", color: "text-blue-400", border: "border-blue-500/20" },
                ].map((s) => (
          ...
          {section === "verify" && (
            <div>
              <div className="text-2xl font-bold mb-6">🔍 Verify Booth Results</div>
              <div className="flex flex-col gap-4 mb-6">
                {stationBooths.map((b) => (
                  <div key={b.id} className={`bg-white/[0.03] border rounded-2xl p-5 ${b.submitted ? "border-green-500/20" : "border-white/10"}`}>
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <div className="font-bold">{b.name}</div>
                        <div className="text-xs text-white/30 mt-0.5">Issued: {b.issued} · Used: {b.used || "—"}</div>
                      </div>
                      <span className={`px-3 py-1 rounded-full text-xs font-bold border ${
                        b.submitted
                          ? "bg-green-500/15 border-green-500/30 text-green-400"
                          : "bg-yellow-500/10 border-yellow-500/20 text-yellow-400"
                      }`}>{b.submitted ? "SUBMITTED" : "PENDING"}</span>
                    </div>
                    {b.submitted && (
                      <div className="grid grid-cols-4 gap-2">
                        {CANDIDATES.map((c) => (
                          <div key={c.id} className="bg-white/[0.04] rounded-xl p-2.5 text-center">
                            ...
                            <div className="font-bold text-yellow-400 font-mono text-sm">{b.candidateVotes[c.id]}</div>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              {!station?.verified ? (
                <div>
                  ...
                  >✓ Verify & Submit Station Result →</button>
                </div>
              ) : (
          ...
          {section === "voters" && (
            <div>
              <div className="text-2xl font-bold mb-6">👤 Voter List</div>
              <div className="bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden">
                <div className="grid grid-cols-4 px-5 py-3 bg-orange-500/[0.07] text-[10px] tracking-widest text-orange-400">
                  <span>NAME</span><span>NID</span><span>BOOTH</span><span>STATUS</span>
                </div>
                {VOTERS.map((v) => (
                  <div key={v.nid} className="grid grid-cols-4 px-5 py-3.5 border-t border-white/5 text-sm hover:bg-white/[0.02]">
                    <span>{v.name}</span>
          ...
          {section === "incidents" && (
            <div>
              <div className="text-2xl font-bold mb-6">🚨 Incident Reports</div>
              <div className="flex flex-col gap-3 mb-8">
                {incidents.map((inc, i) => (
                  <div key={i} className={`bg-white/[0.03] border rounded-2xl p-4 flex gap-4 items-center ${
                    inc.status === "active" ? "border-red-500/20" : inc.status === "pending" ? "border-orange-500/20" : "border-green-500/20"
                  }`}>
                    <div className="font-mono text-yellow-400 text-sm w-12 flex-shrink-0">{inc.time}</div>
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold border flex-shrink-0 ${
                      inc.status === "active" ? "bg-red-500/15 border-red-500/25 text-red-400"
                      : inc.status === "pending" ? "bg-orange-500/15 border-orange-500/25 text-orange-400"
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (!type || !desc) { toast.error("Please select a type and enter a description"); return; }
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-[#0d1117] border border-orange-500/25 rounded-2xl p-9 max-w-sm w-[90%] text-center">
            ...
            <div className="text-xs text-white/30 mb-6">This will lock the station results and forward to the Assistant Returning Officer.</div>
            <div className="flex gap-3">
              ...
              <button onClick={confirmVerify} className="flex-[2] py-3 rounded-xl bg-gradient-to-r from-orange-600 to-orange-700 text-white font-bold text-sm hover:brightness-110 transition-all">✓ Submit</button>
            </div>
          </div>
        </div>
      )}
