...
import { CANDIDATES, CONSTITUENCIES, ELIGIBLE_VOTERS } from "../data/mockData";
...
export default function Guest() {
  ...
  return (
    <div className="min-h-screen bg-[#06090f] text-white">
      {/* topnav */}
      <div className="sticky top-0 z-50 bg-black/60 border-b border-white/10 backdrop-blur px-8 py-3 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <span className="text-2xl">🇧🇩</span>
          <div>
            <div className="text-xs font-bold text-yellow-400 tracking-widest">NATIONAL RESULTS CENTER</div>
            <div className="text-[10px] text-white/30">জাতীয় ফলাফল · Live Feed</div>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
            <span className="text-xs text-red-400 tracking-widest font-bold">LIVE</span>
          </div>
          <span className="font-mono text-xs text-white/30">{time.toLocaleTimeString("en-BD")}</span>
          <button
            onClick={() => navigate("/")}
      ...
      <div className="max-w-5xl mx-auto px-6 py-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {[
            ...
            <div key={s.label} className={`bg-white/[0.03] border ${s.border} rounded-2xl p-4 text-center`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="md:col-span-2 flex flex-col gap-5">
            <div className="bg-white/[0.03] border border-yellow-500/20 rounded-2xl p-6">
              <div className="text-[11px] tracking-widest text-yellow-400 font-bold mb-5">▶ CANDIDATE RESULTS</div>
              <VoteBar />
            </div>

            {/* constituency */}
            <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-5">
              <div className="text-[10px] tracking-widest text-white/30 mb-4">CONSTITUENCY BREAKDOWN</div>
